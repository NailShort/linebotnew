...
export default async (event) => {
  try {
    const { data } = await axios.get('https://www.cmfish.com/mobile/kinds/kindsorts.php?range=1&id=4')
    ...
    $('tbody').each(function () {
      ...
      if (text !== '' && text.length < 10) {
        ...
        Clownfish.push(bubble)
      }
      bubble.body.contents[2].contents[0].contents[1].contents[0].contents[0].text = '分類：雀鯛科'
      // bubble.body.contents[2].contents[0].contents[1].contents[0].contents[0].text = $$('.bbWrapper').text().trim().slice(0, 5)
    })

    const replies = _.chunk(Clownfish, 12).map(Clownfish => {
      return {
        type: 'flex',
        altText: '小丑魚查詢結果',
        contents: {
    ...
    if (process.env.WRITEJSON) {
      for (const i in replies) {
        writejson(replies[i], `Clownfish${i}`)
      }
    }
  } catch (error) {
    console.log(error)
