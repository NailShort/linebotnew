...
export default async (event) => {
  try {
    const { data } = await axios.get('https://www.cmfish.com/mobile/kinds/kindsorts.php?range=1&id=10')
    ...
    $('tbody').each(function () {
      ...
      bubble.body.contents[2].contents[0].contents[1].contents[0].contents[0].text = '分類：剝皮魨科'
      // bubble.body.contents[2].contents[0].contents[1].contents[0].contents[0].text = $$('.bbWrapper').text().trim().slice(0, 5)
    })

    const replies = _.chunk(Gunfish, 12).map(Gunfish => {
      return {
        type: 'flex',
        altText: '剝皮魨查詢結果',
        contents: {
