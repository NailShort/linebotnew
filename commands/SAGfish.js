...
export default async (event) => {
  try {
    const { data } = await axios.get('https://www.cmfish.com/mobile/kinds/kindsorts.php?range=1&id=8')
    ...
    const replies = _.chunk(SAGfish, 12).map(SAGfish => {
      return {
        type: 'flex',
        altText: '小神仙查詢結果',
        contents: {
