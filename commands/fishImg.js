...
import template from '../templates/fishImg.js'
import writejson from '../utils/writejson.js'
import _ from 'loadsh'

export default async (event) => {
  try {
    const { data } = await axios.get('https://www.ph84.idv.tw/forum/forums/617/')
    ...
    const replies = _.chunk(fishImg, 12).map(fishImg => {
      return {
        type: 'flex',
        altText: '魚缸分享查詢結果',
        contents: {
